...
#### BMC не ловит сеть
Скорее всего слетел MAC из прошивки BMC. Првоерить можно выполнив:
```bash
$ sudo ipmitool lan print 1 | grep 'MAC Address'
MAC Address             : 00:00:00:00:00:00
$ sudo ipmitool lan print 8 | grep 'MAC Address'
MAC Address             : 00:00:00:00:00:00
```
Если MAC стоит в нулях, то его нужно восстановить. Процесс максимально простой (2 команды) и [есть ниже](#mac-repair)
...
Полный мануал что и как можно [посмотреть тут](/docs/wiki/hardware/modding-and-hacks/megarac-sp/java-console/index.md), отличие только в том что П1 не нужно делать.
...
Так же нужно передвинуть джампер at/atx питания (см. мануал к материнской плате)

#### Прошивка BIOS через BMC не работает?

Из того что смог исследовать в бинарях, прошивка BIOS вообще не реализована.
...
| ------- | ------- | --------- | -------- |
| 1       | shared     | верхняя наклейка `BMC MAC`          | ethaddr, eth1addr  |
| 8       | dedicated  | он есть в BMC, но физически его НЕТ | -                  |

### MAC repair

На данный момент получилось это сделать простым способом [через ipmitools (вариант с 1 MAC'ом)](/docs/wiki/hardware/modding-and-hacks/megarac-sp/mac-recovery/index.mdx#a-самый-простой-через-ipmitools-вариант-с-1-macом) и более муторным [с изменением UBoot env](/docs/wiki/hardware/modding-and-hacks/megarac-sp/mac-recovery/index.mdx#c-напрямую-редактируем-uboot-env-все-macи).

- Если у вас слетел только с канала 1
  - [через ipmitools (вариант с 1 MAC'ом)](/docs/wiki/hardware/modding-and-hacks/megarac-sp/mac-recovery/index.mdx#a-самый-простой-через-ipmitools-вариант-с-1-macом)
  - [с изменением UBoot env](/docs/wiki/hardware/modding-and-hacks/megarac-sp/mac-recovery/index.mdx#c-напрямую-редактируем-uboot-env-все-macи)
- Если у вас слетел только с канала 8 или с 1 и 8
  - [с изменением UBoot env](/docs/wiki/hardware/modding-and-hacks/megarac-sp/mac-recovery/index.mdx#c-напрямую-редактируем-uboot-env-все-macи)
...
```json title="02_uboot-env.bin"
{
  ...
  "variables": {
    ...
    "bootselector": "1",
    "do_memtest": "0",
    "eth1addr": "f8:cc:6e:03:3b:f2", // change to shared
    "ethact": "ast_eth0",
    "ethaddr": "f8:cc:6e:03:3b:f2",  // change to shared
    "loads_echo": "1",
    "memtest_pass": "idle",
    "recentlyprogfw": "1",
    "stderr": "serial",
...
После любого из способов BMC должен начать получать сеть по DHCP.

## Получение рутового шелла {#root-shell}
