...
- `dmesg` / `/proc/mtd` - разметка (дублирует в большей части данные из FMH)
- `/sys/class/mtd/mtd*/erasesize` - узнать `erasesize`
- `/proc/sys/ractrends/Helper/*` - узнать адрес где лежат `uboot env`

#### `/proc/mtd`

В `/proc/mtd` лежит разметка флешки на блоки и обычно дублирует то что можно и так достать из FMH
...
Пришло время на практике раздербанить дамп на кусочки. Повсеместно будет использован самописный комбайн [uefi-mod-tools](/docs/wiki/hardware/tools/index.md#uefi-mod-tools). Все операции производятся в linux.
...
Дамп можно снять прищёпкой, софтверно через [lxflash (ASPEED iRMP SOC Flash)](/docs/wiki/hardware/tools/index.md#aspeed-irmp-soc-flash), как то софтверно через `flasher` из sh BMC (на практике не делалось).
