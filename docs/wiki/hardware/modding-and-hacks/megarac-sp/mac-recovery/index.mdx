...
В общем случае Megarac SP это просто linux дистр на базе немного переделанного UBoot. MAC адреса хранятся а секции env переменных загрузчика где то после него самого. Что где и как лежит в образе можно почитать в [Deep dive into Megarac SP image](../deep-dive-into-fmh/index)
...
О всех вариантах поиска этой страницы есть в [Deep dive into Megarac SP image](../deep-dive-into-fmh/index). В данном гайде подём по наивному пути с сканом дампа.

Ставим `uefi-mod-tools` по [гайду](/docs/wiki/hardware/tools/uefi-mod-tools/index#install)
